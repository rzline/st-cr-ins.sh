...
detect_system() {
    ...
    if [[ -n "$PREFIX" && "$PREFIX" == *"/com.termux"* ]]; then
        ...
        if ldd --version 2>&1 | grep -qi 'musl' || (command -v readelf &>/dev/null && readelf -l /bin/sh 2>/dev/null | grep -q 'musl'); then
            is_musl=true
            echo "检测到原生 MUSL 环境。将使用 musl 二进制文件。"
        else
            echo "检测到 glibc 环境。"
            local detected_version required_version="2.38" glibc_major glibc_minor req_major req_minor
            ...
            if [[ -n "$detected_version" ]] && [[ "$detected_version" =~ ^([0-9]+)\.([0-9]+)$ ]]; then
                 ...
                 if (( glibc_major > req_major )) || { (( glibc_major == req_major )) && (( glibc_minor >= req_minor )); }; then
                    echo "glibc 版本 ($detected_version) 大于或等于所需版本 ($required_version)。将使用 glibc 二进制文件。"
                    USE_GLIBC_BINARY=true
                 else
                    echo "glibc 版本 ($detected_version) 低于所需版本 ($required_version)。将使用 musl 二进制文件。"
                 fi
            else
                echo "警告：无法从 'ldd --version' 输出中准确解析 glibc 版本号。将使用 musl 二进制文件以保证兼容性。"
            fi
        fi
    else
        echo "警告：缺少 'ldd' 命令，无法检测 libc 类型或版本。将使用 musl 二进制文件以保证兼容性。"
    fi
...
setup_download_url() {
    ...
    if $TERMUX; then
        ...
        echo "检测到 glibc 版本 >= 2.38，选择 glibc 版本。"
    else
        FILENAME="clewdr-musllinux-${ARCH}.zip"
        if [ "$MUSL" = true ]; then
             echo "检测到原生 musl 环境，选择 musl 版本。"
        elif [ -z "$(command -v ldd)" ]; then
             echo "无法找到 ldd 命令，为保证兼容性选择 musl 版本。"
        elif ! grep -qE '[0-9]+\.[0-9]+' <<<"$(ldd --version 2>/dev/null)"; then
             echo "无法确定 glibc 版本，为保证兼容性选择 musl 版本。"
        else
             echo "检测到 glibc 版本低于 2.38，选择 musl 版本。"
        fi
    fi
